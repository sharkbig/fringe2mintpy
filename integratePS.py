...
def export_singleMst_PSDS(networkObj,dateList,inps,ncols,nrows,dsSlc,dsTcor,psDataset,nblocks,linesPerBlock):
    # loop over all pairs
...
def integratePSDS_stamps(dsDS_band, dsSlc, dsTcor, psDataset, outDataset,
                        nblocks, linesPerBlock, bandi):

    nrows = dsDS_band.RasterYSize
    ncols = dsDS_band.RasterXSize
    x0 = 0
    xoff = ncols
    ...
    for block in range(nblocks):
        ...
        psPixels = psDataset.ReadAsArray(x0, y0, xoff, yoff)
        band_i_ds = dsDS_band.ReadAsArray(x0, y0, xoff, yoff)

        band_i_slc = dsSlc.GetRasterBand(bandi).ReadAsArray(x0, y0, xoff, yoff)
        band_i_ds=np.abs(band_i_slc)*band_i_ds
        # get the data for PS pixels
        
        band_i_ds[psPixels == 1] = band_i_slc[psPixels == 1]
        print(band_i_ds.dtype)
        outDataset.GetRasterBand(1).WriteArray(band_i_ds, x0, y0)
...
def exportStamps(dateList,inps,ncols,nrows,dsSlc,dsTcor,psDataset,nblocks,linesPerBlock):
    for date in dateList:    
        
        subDir=os.path.join(inps.outDir,date)
        if not os.path.exists(subDir):
            os.makedirs(subDir)
        # name of the output file witth both PS and DS pixels
        outName = os.path.join(subDir, "{0}.slc".format(date))
        driver = gdal.GetDriverByName("ENVI")
        outDataset = driver.Create(outName, ncols, nrows, 1, gdal.GDT_CFloat32)
        
        # out dataset DS dataset 
        band = dateList.index(date) + 1
        dsDS_band = gdal.Open(os.path.join(inps.dsStackDir, date + ".slc.vrt"), gdal.GA_ReadOnly)
        
        # integrate PS to DS for this pair and write to file block by block
        integratePSDS_stamps(dsDS_band, dsSlc, dsTcor, psDataset, outDataset,
                        nblocks, linesPerBlock, band)
...
def main(iargs=None):
    ...
    if inps.stamps:
        exportStamps(dateList,inps,ncols,nrows,dsSlc,dsTcor,psDataset,nblocks,linesPerBlock)
    else:
